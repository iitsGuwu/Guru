...
// Force-dynamic so on-chain reads don't run at build time. The lib reads
// are wrapped in `unstable_cache` for per-request reuse.
  ...
  return (
    <div className="relative z-10 mx-auto max-w-[2000px]">
      ...
      {/* Two-column desktop layout: sticky artwork on the left, scrolling
          metadata + bid panel on the right. */}
      <div className="grid grid-cols-1 lg:grid-cols-[2fr_1fr] min-h-[calc(100vh-64px)] mt-6">
        <div className="lg:sticky lg:top-6 lg:h-[calc(100vh-48px)] flex items-center justify-center bg-surface p-8 lg:p-12">
          <TokenMedia src={image} title={title} />
        </div>

        <aside className="lg:border-l border-white/10 bg-surface px-6 py-8 lg:px-8 lg:py-10">
